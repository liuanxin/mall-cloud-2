...
/**
 * 消息队列模块的配置数据. 主要是 mybatis 的多配置目录和类型处理器
 */
@Configuration
public class QueueWebConfig extends WebMvcConfigurationSupport {

    @Value("${online:false}")
    private boolean online;

    private final ObjectMapper objectMapper;
    public QueueWebConfig(ObjectMapper objectMapper) {
    ...
    public void addInterceptors(InterceptorRegistry registry) {
        registry.addInterceptor(new QueueInterceptor()).addPathPatterns("/**");
    }
}
